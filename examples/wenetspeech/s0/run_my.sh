...
wenetspeech_data_dir=/data/wenetspeech/untar #/ssd/nfs07/binbinzhang/wenetspeech
# Make sure you have 1.2T for ${shards_dir}
shards_dir=/data/wenetspeech_shards_new #/ssd/nfs06/unified_data/wenetspeech_shards
...
dict=data_new/dict/lang_char.txt
decoding_chunk_size=
ctc_weight=0.5
reverse_weight=0.0
blank_penalty=0.0
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    tools/text2token.py -s 1 -n 1 --space "▁" data_new/${train_set}/text \
        | cut -f 2- -d" " | tr " " "\n" \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  full_size=`cat data_new/${train_set}/wav.scp | wc -l`
  sampling_size=$((full_size / cmvn_sampling_divisor))
  shuf -n $sampling_size data_new/$train_set/wav.scp \
    > data_new/$train_set/wav.scp.sampled
  python3 tools/compute_cmvn_stats.py \
  --num_workers 16 \
  --train_config $train_config \
  --in_scp data_new/$train_set/wav.scp.sampled \
  --out_cmvn data_new/$train_set/global_cmvn
fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  for x in $dev_set $test_sets ${train_set}; do
    ...
    tools/make_shard_list.py --resample 16000 --num_utts_per_shard 1000 \
      --num_threads 32 --segments data_new/$x/segments \
      data_new/$x/wav.scp data_new/$x/text \
      $(realpath $dst) data_new/$x/data.list
  done
fi
